...
* Head **`0.2.41`** (service persists paired owner in secrets.bin — no more fake pairing prompt / visible tray after reboot). Installer: `windows/installer/Output/DeviceServiceSetup-0.2.41.exe` (folds 0.2.37 uninstaller fix + 0.2.38 sync-heal + this). Phone APK: `release-apk/LAPTOP-PARENT-0.2.40-release.apk` (0.2.39 skipped; no phone-only release since). Rules: `customSites` registry node deployed live to SG. Web console: `https://guardpulse-laptop-sg.web.app` (§15).
