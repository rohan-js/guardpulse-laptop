...
* Head **`0.2.40`** (custom-sites toggle OFF = blocked + remove confirmation), pushed. Installer: `windows/installer/Output/DeviceServiceSetup-0.2.38.exe` (folds in 0.2.37's uninstaller .msg fix + net start retry — 0.2.39/0.2.40 changed phone/rules only, no agent). Phone APK: `release-apk/LAPTOP-PARENT-0.2.40-release.apk` (0.2.39 was never installed on the phone — skip it). Rules: `customSites` registry node deployed live to SG.
